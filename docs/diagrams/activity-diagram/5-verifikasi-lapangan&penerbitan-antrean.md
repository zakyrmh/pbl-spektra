...
classDef kondisi fill:#ff9,stroke:#333,stroke-width:1px;
    ...
    %% GERBANG KEPUTUSAN VERIFIKASI
    FO2 --> FO3{"Apakah Dokumen\nValid & Pengunjung\nBerhak Dilayani?"}:::kondisi

    %% JALUR JIKA DITOLAK
    FO3 -->|Tidak / Tolak| S2["Sistem: Update Status Booking Menjadi 'Cancelled' di DB"]
    ...
    %% JALUR JIKA DISETUJUI — OTOMATIS GENERATE ANTREAN
    FO3 -->|Ya / Setujui| S5["Sistem: Update Status Booking Menjadi 'Confirmed'"]
    ...
    S7 --> S8["Sistem: Simpan Data ke Tabel Queues dengan Status 'Waiting'"]

    %% BROADCAST REAL-TIME KE COUNTER
    S8 --> S9["Sistem: Broadcast Data Antrean Baru ke Dashboard Admin Counter Terkait via Websocket"]
    ...
    FO4 --> FO5["Admin FO: Mengarahkan Pengunjung Menuju Ruang Tunggu Counter"]
    FO5 --> End
```
