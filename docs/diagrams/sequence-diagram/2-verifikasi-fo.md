...
sequenceDiagram
    ...
    participant LK as Dashboard Admin Counter/Gerai
    ...
    FO->>BE: Klik Tombol "Approve & Check-in"

    Note over BE: Menjalankan DB::transaction()
    BE->>DB: UPDATE bookings SET status = 'Confirmed'<br/>INSERT INTO queues (queue_number, status: 'Waiting', queue_date, ...)
    ...
    and Broadcast Real-time ke Counter Gerai
