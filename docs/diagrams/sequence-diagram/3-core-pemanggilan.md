...
sequenceDiagram
    autonumber
    actor LK as Admin Loket / Gerai
    participant FE_LK as Dashboard Loket (Frontend)
    participant BE as Laravel API (LoketController)
    ...
    BE->>DB: UPDATE queues SET status = 'Dipanggil', started_at = NOW()
    DB-->>BE: Confirm Save
    ...
    PV->>LK: Berjalan Menuju Loket & Menerima Pelayanan Staf

    LK->>FE_LK: Proses Selesai -> Klik Tombol "Selesai Dilayani"
    FE_LK->>BE: POST /api/queues/{id}/finish
    BE->>DB: UPDATE queues SET status = 'Selesai', ended_at = NOW()
    DB-->>BE: Confirm Save
