...
Dokumen ini menyediakan panduan langkah-demi-langkah untuk memasang dan menjalankan **Sistem Manajemen Antrean MPP Kota Sawahlunto (Spektra)** di lingkungan lokal menggunakan **Laragon** pada sistem operasi Windows.

---

## 📋 Persyaratan Sistem (System Requirements)

Sebelum memulai instalasi, pastikan lingkungan pengembangan lokal Anda memenuhi spesifikasi minimum berikut:

- **Sistem Operasi**: Windows 10 / 11
- **Lingkungan Web Server**: Laragon (disarankan menggunakan versi Full yang menyertakan Apache/Nginx & MySQL)
- **PHP**: Versi 8.2 atau 8.3 (dilengkapi dengan ekstensi `mbstring`, `xml`, `bcmath`, `gd`, `openssl`, `pdo_mysql`)
- **Database**: MySQL 8.0+ atau MariaDB 10.4+ (bawaan Laragon)
- **Dependency Manager**: 
  - Composer (PHP) versi 2.6+
  - Node.js (Javascript/Vite) versi 18.x atau 20.x (disertai NPM)
- **WebSocket**: Akun Pusher (untuk development) atau instalasi Soketi lokal
...
composer install
```
*Catatan: Pastikan PHP versi 8.2+ sudah aktif di Laragon. Anda bisa mengubah versi PHP melalui menu klik kanan Laragon -> PHP -> Version.*
...
3. Konfigurasikan koneksi database Anda di bagian berikut:
   ...
   DB_PASSWORD=
   ```
   *(Secara default, Laragon menggunakan DB_USERNAME `root` dan DB_PASSWORD kosong)*
...
- Jika menggunakan virtual host `http://pbl-spektra.test`, pastikan url di browser sudah benar dan tidak ada eror sertifikat SSL di browser (Anda bisa menonaktifkan paksaan HTTPS di Laragon klik kanan -> Apache/Nginx -> SSL -> Enabled).
...
- Buka Console Developer Tools di browser (F12) dan periksa tab *Console* apakah ada eror koneksi Pusher.
