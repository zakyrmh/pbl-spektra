...
<div class="max-w-4xl mx-auto space-y-6 pb-16">
        ...
        <div>
            <div class="flex items-center gap-2 mb-1">
                <span class="relative flex h-2.5 w-2.5">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-status-serving opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-status-serving"></span>
                </span>
                <span class="text-[11px] font-bold text-status-serving uppercase tracking-widest font-display">Stasiun Check-In Aktif</span>
            </div>
            <h1 class="text-2xl font-bold text-ink dark:text-white font-display">Verifikasi & Check-In</h1>
            <p class="text-sm text-muted dark:text-on-dark-soft font-body mt-0.5">Scan atau ketik kode booking warga untuk verifikasi kedatangan.</p>
        </div>
        <div class="text-xs text-muted dark:text-on-dark-soft font-mono bg-surface-soft dark:bg-white/5 border border-hairline dark:border-white/10 px-3 py-1.5 rounded-md" id="checkin-clock">
            Loading waktu...
        </div>
    </div>
    ...
    {{-- Success --}}
    @if(session('success'))
    <div class="flex items-start gap-3 p-4 bg-status-serving/10 border border-status-serving/30 rounded-lg" role="alert" id="alert-success">
        <svg class="w-5 h-5 text-status-serving shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    ...
    {{-- Error --}}
    @if(session('error'))
    <div class="flex items-start gap-3 p-4 bg-status-skipped/10 border border-status-skipped/30 rounded-lg" role="alert" id="alert-error">
        <svg class="w-5 h-5 text-status-skipped shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        <div class="flex-1 min-w-0">
    ...
    @if(session('warning'))
    <div class="flex items-start gap-3 p-4 bg-status-waiting/10 border border-status-waiting/30 rounded-lg" role="alert" id="alert-warning">
        <svg class="w-5 h-5 text-status-waiting shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
        <div class="flex-1 min-w-0">
        ...
        <button onclick="this.closest('[role=alert]').remove()" class="shrink-0 text-amber-600 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-200 transition-colors cursor-pointer">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
    </div>
    @endif
    ...
    {{-- Validation Errors --}}
    @if($errors->any())
    <div class="flex items-start gap-3 p-4 bg-status-skipped/10 border border-status-skipped/30 rounded-lg" role="alert" id="alert-validation">
        <svg class="w-5 h-5 text-status-skipped shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        <div class="flex-1 min-w-0">
    ...
    ═══════════════════════════════════════════════════════════════ --}}
    <div class="bg-canvas dark:bg-surface-dark-elevated p-6 sm:p-8 rounded-xl border border-hairline dark:border-white/10 shadow-sm">
        <div class="flex items-center gap-2 mb-4">
            <svg class="w-5 h-5 text-primary dark:text-accent-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
            <h2 class="text-lg font-bold text-ink dark:text-white font-display">Scan atau Ketik Kode Booking</h2>
        </div>
        <p class="text-sm text-muted dark:text-on-dark-soft font-body mb-5">Arahkan scanner ke QR/barcode tiket warga, atau ketik manual kode booking (UUID) di kolom berikut.</p>
        ...
        <form action="{{ route('admin.fo.checkin.verify') }}" method="POST" id="formCheckin" autocomplete="off">
            ...
            <p class="text-xs text-muted dark:text-on-dark-soft mt-2.5 font-body">
                <span class="font-semibold">Tip:</span> Kolom ini siap menerima input dari hardware barcode/QR scanner secara otomatis.
            </p>
        </form>
    </div>
    ...
    <div class="bg-canvas dark:bg-surface-dark-elevated rounded-xl border border-hairline dark:border-white/10 shadow-sm overflow-hidden" id="nik-panel">

        {{-- Amber Banner --}}
        <div class="flex items-start gap-3 p-4 sm:p-5 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-700/40">
            <div class="p-2 bg-amber-100 dark:bg-amber-800/30 rounded-lg shrink-0">
                <svg class="w-6 h-6 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
            </div>
            <div>
                ...
                <p class="text-sm text-amber-700 dark:text-amber-400 font-body mt-0.5">
                    Data NIK warga <strong>{{ $bk->user->name }}</strong> belum terisi di sistem. Silakan minta KTP warga dan isikan NIK di bawah ini sebelum melanjutkan check-in.
                </p>
            </div>
        </div>
        ...
        <div class="p-5 sm:p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    ...
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-status-waiting/12 text-status-waiting rounded-full text-[11px] font-bold">
    ...
    <div class="bg-canvas dark:bg-surface-dark-elevated rounded-xl border-2 border-status-serving/40 shadow-sm overflow-hidden" id="checkin-result-card">
        ...
        <div class="p-5 sm:p-6">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    <span class="block text-[11px] font-bold text-muted dark:text-on-dark-soft uppercase tracking-wider font-display mb-1">Kode Booking</span>
                    <span class="font-mono font-semibold text-primary dark:text-accent-teal">{{ $cr->booking_code }}</span>
                </div>
                <div>
                    <span class="block text-[11px] font-bold text-muted dark:text-on-dark-soft uppercase tracking-wider font-display mb-1">NIK</span>
                    <span class="font-mono text-ink dark:text-white">{{ $cr->user->nik ?? '-' }}</span>
                </div>
                <div>
                    <span class="block text-[11px] font-bold text-muted dark:text-on-dark-soft uppercase tracking-wider font-display mb-1">Waktu Check-In</span>
                    <span class="font-mono text-ink dark:text-white">{{ $cr->checked_in_at?->format('H:i:s') }}</span>
                </div>
            </div>

            <div class="mt-4 pt-4 border-t border-hairline dark:border-white/10 flex items-center gap-2">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-status-serving/12 text-status-serving rounded-full text-xs font-bold">
                    <span class="w-1.5 h-1.5 rounded-full bg-status-serving"></span>Checked-In
                </span>
                <span class="text-xs text-muted dark:text-on-dark-soft font-body">— Warga siap menunggu panggilan antrean.</span>
            </div>
        </div>
    </div>
    @endif
    ...
    {{-- ═══════════════════════════════════════════════════════════════
         PETUNJUK PENGGUNAAN
    ═══════════════════════════════════════════════════════════════ --}}
    <div class="bg-canvas dark:bg-surface-dark-elevated p-5 sm:p-6 rounded-xl border border-hairline dark:border-white/10 shadow-sm">
        <h3 class="text-sm font-bold text-ink dark:text-white font-display mb-3">Petunjuk Penggunaan</h3>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="flex gap-3">
                <div class="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/20 text-primary dark:text-accent-teal flex items-center justify-center text-sm font-bold font-mono">1</div>
                <div>
                    <p class="text-sm font-semibold text-ink dark:text-white font-display">Terima Kode</p>
                    <p class="text-xs text-muted dark:text-on-dark-soft font-body mt-0.5">Minta warga menunjukkan QR code atau kode booking dari email/WA.</p>
                </div>
            </div>
            <div class="flex gap-3">
                <div class="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/20 text-primary dark:text-accent-teal flex items-center justify-center text-sm font-bold font-mono">2</div>
                <div>
                    <p class="text-sm font-semibold text-ink dark:text-white font-display">Scan / Ketik</p>
                    <p class="text-xs text-muted dark:text-on-dark-soft font-body mt-0.5">Arahkan scanner ke QR code, atau ketik kode manual di kolom input.</p>
                </div>
            </div>
            <div class="flex gap-3">
                <div class="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/20 text-primary dark:text-accent-teal flex items-center justify-center text-sm font-bold font-mono">3</div>
                <div>
                    <p class="text-sm font-semibold text-ink dark:text-white font-display">Verifikasi</p>
                    <p class="text-xs text-muted dark:text-on-dark-soft font-body mt-0.5">Sistem memvalidasi data. Jika NIK kosong, isi di tempat lalu check-in.</p>
                </div>
            </div>
        </div>
    </div>
