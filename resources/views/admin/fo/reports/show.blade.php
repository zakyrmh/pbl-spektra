...
@section('title', $report->title . ' — Detail Laporan Kinerja')
    ...
    <div class="max-w-7xl mx-auto space-y-6 pb-16">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.fo.reports.index') }}"
                    ...
                    <div class="flex items-center gap-2 mb-1">
                        @if ($report->status === 'Terkirim')
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 text-[10px] font-bold uppercase rounded bg-status-serving/10 text-status-serving tracking-wider select-none">
                                Terkirim ke Super Admin
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 text-[10px] font-bold uppercase rounded bg-status-waiting/10 text-status-waiting tracking-wider select-none">
                                Draft / Belum Dikirim
                            </span>
                        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-canvas dark:bg-surface-dark-elevated p-6 rounded-lg border border-hairline dark:border-white/10 shadow-xs space-y-4">
                ...
                <div id="chartDailyTrend" class="w-full min-h-75"></div>
            </div>
            ...
            <div class="bg-canvas dark:bg-surface-dark-elevated p-6 rounded-lg border border-hairline dark:border-white/10 shadow-xs space-y-4">
                ...
                <div id="chartGeraiDistribution" class="w-full min-h-75"></div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const chartGeraiOptions = {
            ...
            colors: ['#1B4FA8', '#29ABE2', '#059669', '#D97706', '#8B5CF6'],
            dataLabels: {
                enabled: true,
                textAnchor: 'start',
                style: {
