...
@section('auth_content')
    <div class="mb-8">
        ...
        <p class="text-body-sm text-muted mt-1 font-body">Sistem Antrean Digital Mall Pelayanan Publik</p>
    </div>
    ...
    {{-- Alert sukses --}}
    @if (session('status'))
        <div class="mb-5 text-body-sm text-status-serving bg-status-serving/5 border border-status-serving/20 rounded-lg px-4 py-3 font-body">
            {{ session('status') }}
        </div>
    @endif
    ...
    <form action="{{ route('login.process') }}" method="POST" class="space-y-5">
        ...
        {{-- Email --}}
        <div>
            <label class="block text-title-sm font-semibold text-ink mb-2 font-body">
                NIK atau Email
            </label>
            <div class="relative">
        ...
        <div>
            <div class="flex justify-between items-center mb-2">
                <label class="block text-title-sm font-semibold text-ink font-body">Kata Sandi</label>
                <a href="{{ route('password.request') }}"
                    class="text-body-sm font-semibold text-primary hover:text-primary-hover transition-colors font-body">
                    Lupa Password?
                </a>
            </div>
            <div class="relative">
                ...
                <button type="button" onclick="togglePassword()"
                    ...
                    <svg id="eye-open" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.964-7.178z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    <svg id="eye-closed" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5 hidden" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="remember" id="remember"
                class="w-4.5 h-4.5 border border-hairline rounded-sm bg-canvas text-primary focus:ring-3 focus:ring-primary/12 focus:border-primary transition cursor-pointer">
            <label for="remember" class="text-body-sm text-body font-body cursor-pointer">Ingat saya di perangkat ini</label>
        </div>

        {{-- Submit --}}
        <button type="submit"
            class="w-full h-11 flex items-center justify-center gap-2 px-6 bg-primary hover:bg-primary-hover active:scale-[0.98] text-white text-button font-semibold rounded-pill shadow-md transition-all duration-150 cursor-pointer">
            Masuk
    ...
    <div class="flex items-center gap-3 my-5">
        ...
        <span class="text-caption text-muted font-medium font-body">ATAU</span>
        <div class="flex-1 h-px bg-hairline-soft"></div>
    </div>
