...
@section('auth_content')
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-500 mt-1">Silakan masukkan password baru Anda di bawah ini</p>
    </div>
    ...
    <form action="{{ route('password.update') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="email" name="email" id="email" value="{{ request()->email ?? old('email') }}"
                    readonly
                    class="w-full pl-10 pr-4 py-3 text-sm border {{ $emailClass }} rounded-xl text-gray-500 bg-gray-100 focus:outline-none transition cursor-not-allowed">
            </div>
            @error('email')
        ...
        {{-- Submit --}}
        <button type="submit"
            class="w-full py-3.5 bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white text-sm font-bold rounded-xl shadow-md shadow-blue-200 transition-all duration-150 flex items-center justify-center gap-2 mt-2">
