...
@section('auth_content')
    <div class="mb-8">
        ...
        <p class="text-body-md text-muted mt-2 font-body">Silakan masukkan password baru Anda di bawah ini</p>
    </div>
    ...
    <form action="{{ route('password.update') }}" method="POST" class="space-y-5">
            ...
            <div class="relative">
                ...
                @php 
                    $emailClass = $errors->has('email') 
                        ...
                        : 'border border-hairline bg-surface-soft text-muted cursor-not-allowed'; 
                @endphp
                <input type="email" name="email" id="email" value="{{ request()->email ?? old('email') }}"
                    readonly
                    class="w-full pl-10 pr-4 py-3 text-body-md {{ $emailClass }} rounded-md placeholder-muted-soft focus:outline-none transition font-body">
            </div>
            @error('email')
        ...
        {{-- Password Baru --}}
        <div>
            <label for="password" class="block text-title-sm font-semibold text-ink mb-2 font-body">Password Baru</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-3.5 flex items-center text-muted-soft">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="2">
                ...
                @endphp
                <input type="password" name="password" id="password" placeholder="Minimal 8 karakter" autofocus required
        ...
        {{-- Konfirmasi Password --}}
        <div>
            <label for="password_confirmation" class="block text-title-sm font-semibold text-ink mb-2 font-body">Konfirmasi Password Baru</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-3.5 flex items-center text-muted-soft">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="2">
        ...
        {{-- Submit --}}
        <button type="submit"
            class="w-full h-11 flex items-center justify-center gap-2 px-6 bg-primary hover:bg-primary-hover active:scale-[0.98] text-white text-button font-semibold rounded-pill shadow-sm transition-all duration-150 cursor-pointer mt-2">
