...
// Fungsi pembantu untuk membuat data awal
function createTestData()
{
    ...
    return compact('department', 'service', 'counter', 'visitorUser', 'foAdmin');
}

test('guests are redirected to login when accessing walkin ticket routes', function () {
    test()->get(route('admin.fo.ticket.create'))->assertRedirect(route('login'));
    test()->post(route('admin.fo.ticket.store'), [])->assertRedirect(route('login'));
});

test('visitor role cannot access walkin ticket routes (403)', function () {
    $data = createTestData();

    test()->actingAs($data['visitorUser'])->get(route('admin.fo.ticket.create'))->assertStatus(403);
    test()->actingAs($data['visitorUser'])->post(route('admin.fo.ticket.store'), [])->assertStatus(403);
});

test('FO admin can view the walkin ticket creation form', function () {
    $data = createTestData();

    $response = test()->actingAs($data['foAdmin'])->get(route('admin.fo.ticket.create'));
